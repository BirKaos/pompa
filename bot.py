...
def kanala_uye_mi(user_id):
    # Admin için zorunluluğu baypas edebiliriz
    if str(user_id) == ADMIN_CHAT_ID:
        return True
    try:
        durum = bot.get_chat_member(KANAL_USERNAME, user_id).status
        # Telegram'da kanalda olanların statüleri: creator, administrator, member
        if durum in ['creator', 'administrator', 'member']:
            return True
    except Exception as e:
        print.error(f"Kanal kontrol hatası: {e}")
    return False
...
def admin_callbacks(call):
    ...
    if call.data == "admin_premium_ekle":
    ...
    elif call.data == "admin_premium_liste":
        bot.answer_callback_query(call.id)
